...
**玩乎不是单纯的文章可视化插件。** 它首先是一套面向知乎创作者的互动内容创作工具：作者可以从自己的文章或回答中选取一段材料，构思互动方式，生成或编辑一个完整演示，预览读者效果，再把玩乎分享链接主动插入知乎正文。读者看到的互动内容，来自作者已经创作并发布的作品；读者也可以在此基础上生成自己的理解版本。\n\n**玩乎要补上的，是解释与理解之间那一步。**
...
知乎负责沉淀问题、回答和经验，玩乎负责让创作者把其中一个观点做成可操作的作品，并让读者在原文中直接使用它。
...
作者完成创作并核对后，把普通玩乎链接插入自己的知乎文章。安装玩乎的读者看到的是文章旁边的作者互动作品；未安装插件的读者仍可点击链接进入玩乎阅读页。
...
轻量辅助直接给出解释、类比或论证检查，不必为一个小问题生成整份作品。作者把问题收敛为“让读者看到每个报文抵达后，两端状态怎样变化”，再进入互动生成与核对。一般生成可以采用 SVG 分镜组织报文移动；评审页另有直接编写的 TCP 专用示例，展示更完整的手动发送与状态变化。
...
*亲手发送报文，观察浏览器和服务器状态变化。截图取自当前线上版本。*
...
*真实知乎原文与玩乎插件自动嵌入实拍。保留原作者信息，并单独标明玩乎预制身份。*

### 6.1.2 从创作到插入：作者作品与读者理解清楚可辨

| 情形 | 如何出现 | 内容归属与发布边界 |
|---|---|---|
| 作者创作并在文章中附带玩乎分享链接 | 插件识别链接并在对应正文附近展开互动卡片 | 作者主动创作、核对并发布的作品；插件不代作者发布文章 |
...
- 知乎 OAuth：提供授权跳转、基础资料读取、退出登录和回到当前作品的入口；
...
- 插件与网页已覆盖作者、读者、预制演示和分享接续等主要体验路径；评审可通过官网和插件直接体验。

当前 OAuth 凭证已配置，授权入口可跳转知乎；完整账号授权回调仍需真实用户登录验收。正文、插件与工坊通过作品快照接续，尚不是云端共享版本服务。手作演示与实时 AI 输出分别展示。
