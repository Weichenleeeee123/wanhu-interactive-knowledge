...
> 文档版本：2026 年 9 月 14 日 · 正式上线版（插件 0.4.3）
...
安装 0.4.3 插件后，打开十篇指定原文即可体验预制作品，不需要选段、等待模型或手动插入。没有插件时，点击本节网页入口同样可以操作。自动嵌入只在本机浏览器增加一层展示，不改变知乎保存的原文。
...
- 知乎 OAuth：代码已具备授权跳转、`state` 防重放、`/user` 基础资料读取、退出登录和授权用户数据入口，App ID / App Key 已配置并同步到新 VPS，正式入口可跳转知乎授权页；登录后返回当前作品，真实用户授权回调尚需登录验收；
...
- 本地 114 项单元测试、52 项网页测试、16 项插件测试通过，类型检查及生产构建通过；
- 2026 年 9 月 14 日已部署至 `https://wanhu.asia`，插件版本 0.4.3；
- 正式域名十篇专用交互验收 8 项通过，覆盖十篇页面、390px 宽度与分享还原；本轮新增 6 项流程回归通过，覆盖文章导入、跨浏览器分享、动画播放和角色接续；线上插件包与发布包校验一致；
- 十篇真实知乎移动阅读页面在本机逐一验证，10/10 自动嵌入。该结果与正式域名测试分别记录，不当作所有知乎页面始终可访问的保证。

详细发布证据见 [部署与验收记录](../docs/deployment-2026-09-14.md)。当前 OAuth 凭证已配置，授权入口跳转已验证，真实用户授权回调尚需登录验收；正文、插件与工坊通过作品快照接续，尚不是云端共享版本服务。手作演示的视觉完成度与实时 AI 输出质量分别展示，不合并宣传。
...
本轮按知乎插件与网页、创作者与读者逐条检查，修复了 SVG 首帧播放崩溃、真实知乎链接导入、读者新建角色丢失、知乎包装外链自动识别、正文演示与侧栏列表接续、选段后编辑问题丢失材料和窄屏遮挡。另在真实知乎 TCP 原文完成了线上生成、自动插入、跳转网页继续理解的完整读者链路；实时接口失败时保留材料并支持重试。已有插件需要更新至 0.4.3 并刷新知乎。详细流程、真实测试与能力边界见 [两端两用户验收记录](../docs/journey-audit-2026-09-14.md)。


### 让第一次使用，也能顺着走下去

玩乎把注意力留给知识本身：空白工坊只突出“准备材料”和“想弄懂什么”，备份和副本收在更多操作里；生成时显示真实等待时间，可以停止等待，原文与问题都保留。结果出现后直接带到预览，插件里则自动切到本次演示，并把按钮改成“查看正文中的演示”，不让用户猜测是否还要再插入一次。

读者可以选择“直接动手试试”或“我有个问题”，不用先读完一整页说明。作者分享时看到粘贴到知乎文章的步骤，读者分享时看到无需安装插件的网页阅读说明。辅助构思仍是随手可用的可选入口。刘看山在阅读页和工坊退到页尾，让出阅读和操作空间。

OAuth 凭证已从本地同步到正式服务器，页面增加知乎登录入口，授权返回地址携带当前作品与角色；登录不被宣传成云端同步。详情见 [本轮体验优化](../docs/ux-refinement-2026-09-14.md)。
