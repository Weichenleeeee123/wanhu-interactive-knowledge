...
test.beforeEach(()=>{requests=[];fail=false;});
...
test('reader selects, reviews, generates, embeds, interacts and restores saved cards',async()=>{
  ...
  await page.getByRole('checkbox').check();await page.getByRole('button',{name:'生成这段的互动演示 ↗'}).click();
...
test('creator preview stays outside the editor and sends only the selected text; errors can retry',async()=>{
  ...
  await page.getByRole('checkbox').check();fail=true;
  ...
  await workshop.waitForURL('**/create#v1.*');expect(workshop.url()).toContain('http://localhost:3014/create#v1.');
