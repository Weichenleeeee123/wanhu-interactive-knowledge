...
export function PredictionCard({
  ...
  if (experiment.type === "article-exploration")
    return (
      <div className="prediction-card">
        <h3>读之前，你对这个问题有多大把握？</h3>
        <div className="reading-initial-options">
          {["我能解释核心观点", "我理解了一部分", "我还没弄明白"].map(
            (option) => (
              <button
                key={option}
                disabled={value !== null}
        ...
        {value && (
          <p className="prediction-confirmation" role="status">
            已记下。带着这个起点，对照材料继续探索。
          </p>
        )}
      </div>
...
export function LearningRecap({
  ...
  function download() {
    const content = [
      ...
      "本次学习目标：" + lesson.goal,
      "",
      "带走一个理解：" + lesson.challenge,
      "",
      "材料来源：",
      ...lesson.sources.map(
        (s) => `${s.title} / ${s.author || "作者未提供"}\n${s.url}`,
      ),
      "",
      "这是个人页面操作记录，不代表对学习效果的评估。",
    ].join("\n");
    ...
    link.href = url;
    link.download = "玩乎-我的探索记录.txt";
    link.click();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
  }
  return (
    <section
      className={`learning-recap ${complete ? "is-complete" : ""}`}
      aria-label="我的探索记录"
    >
      <div className="section-line">
        <div>
          <span className="eyebrow">YOUR EXPLORATION</span>
          <h2>{complete ? "把这次理解，带走。" : "我的探索记录"}</h2>
        </div>
      ...
      <ol>
        <li>
          <span>{reading ? "01 · 阅读起点" : "01 · 我的直觉"}</span>
          <strong>
            {prediction ??
              (reading ? "留下阅读前的理解程度" : "在实验前留下一个预测")}
          </strong>
        </li>
        <li>
          <span>02 · 我做了什么</span>
          <strong>
            {activity || (reading ? "完成一次情境选择" : "动手操作一次实验")}
          </strong>
        </li>
        <li>
          <span>{reading ? "03 · 对照原文" : "03 · 规则验证"}</span>
          <strong>
            {verified === true
              ? reading
                ? "已理解各个情境中的表达"
      ...
      <div className="recap-footer">
        <p>仅记录本次页面体验，不上传个人学习记录。</p>
        <button
          className="button"
          disabled={!prediction && !activity && verified === null}
          onClick={download}
        >
          保存探索记录 ↓
        </button>
