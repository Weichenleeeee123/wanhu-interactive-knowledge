...
import { oauthSessions,sessionId,cookie } from "@/lib/server/oauth";
...
export async function GET(request:Request){
 ...
 const sid=sessionId(),state=sessionId();oauthSessions.set(sid,{state,createdAt:Date.now()});
 ...
 const response=NextResponse.redirect(url);response.headers.append("Set-Cookie",cookie("wanhu_oauth",sid,600));return response;
}
