  ...
  return <div className="zw-shell">
    ...
    {open&&<aside className="zw-panel" ref={panel} tabIndex={-1} aria-label="玩乎工作区" onKeyDown={event=>{if(event.key==='Escape'){setOpen(false);launcher.current?.focus();}}}>
      ...
      <div className="zw-scroll">
        ...
        <footer className="zw-footer"><strong>玩乎如何融入知乎</strong><small>插件版本 0.4.4 · <a href={__BACKEND_URL__+"/extension"} target="_blank" rel="noreferrer">更新与安装说明 ↗</a></small><p>作者在写作时生成互动演示，把分享链接贴进文章；读者打开文章后，玩乎会在链接附近自动展开。正文、作者署名和知乎来源始终保留。</p><small>卡片标注“作者附带”时，表示它来自文章中的玩乎分享链接；读者自己生成的解释只保存在自己的浏览器中。</small></footer>
      </div>
    </aside>}
  </div>;
}
