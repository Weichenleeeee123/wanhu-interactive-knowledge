...
function buildPrompt(input: GenerateInput, repairMessage?: string): string {
  ...
  return [
    ...
    '根据材料选择互动形式：涉及标准梯度下降时用 gradient-descent（initialX 为 -10 至 10；learningRate 为 0.02 至 1.2，步长 0.01）；标准三门问题用 monty-hall（trials 只能是100或1000）。其他包含具体观点、解释、建议或步骤的文章，使用 article-exploration。只有材料不足以支持可核对的理解题时，才输出 {"unsupported":true,"reason":"简短原因"}，不可因为主题不是数学而拒绝。',
