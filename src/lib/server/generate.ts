...
function buildPrompt(input: GenerateInput, repairMessage?: string): string {
  ...
  return [
    ...
    `完整 gradient-descent 输出示例：${JSON.stringify({ ...baseExample, experiment: { type: "gradient-descent", initialX: 0, learningRate: 0.2 } })}`,
...
function validateAndRestore(
  ...
  if (Buffer.byteLength(rawText, "utf8") > 128 * 1024) {
